...
# Copyright (c) 2001-2006, 2017 ActiveState Software Inc.
#
...
# pref::prefList
#
#	List preferences matching a pattern.
#
# Arguments:
#	pref	The pattern to match
#	group	Explicitly specify which group to query
#
# Results: 
#	Return the actual value for the preference.  If the preference
#	does not exist, then an error is generated.

proc ::pref::prefList {pattern {group {}}} {

    if {$group eq ""} {
	set groups [getGroupOrder]
    } else {
    ...
    foreach g $group {
	foreach k [array names ::pref::value${g} $pattern] {
	    lappend results $k
	}
    }

    return [lsort -uniq $results]
}
