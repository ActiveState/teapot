...
#  This file defines four procs, which are both compiled, inside a catch
#  statement; the last command in the catch triggers an error.
#  We also define enough variables to push us past 256 objects in the object
#  table.
...
#  right. In this case, the exception range starts before the place where
#  bytecodes are shifted, so that the offset does not change. But the size of
#  the range does, because of the extra bytes. If the size does not get
#  changed, the last command slips outside the range, and is no longer caught.
...
# All rights reserved.
# 
# RCS: @(#) $Id: proccatch2.tcl,v 1.2 2000/05/30 22:19:12 wart Exp $
...
# the error in the last command should be caught by the catch.
